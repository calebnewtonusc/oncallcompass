...
from core.incident_taxonomy import IncidentCategory, FAILURE_SIGNATURES
...
class RankedHypothesis:
    ...
    causal_chain: list[str]     # e.g., ["redis_down", "cache_miss", "db_overload", "api_5xx"]
...
class HypothesisAgent:
    ...
    def rank(
        ...
        if rule_hypotheses:
            logger.info(f"Rule-based pre-ranking found {len(rule_hypotheses)} hypotheses")

        prompt = self._build_prompt(incident, service_graph, rule_hypotheses)

        logger.info(f"Ranking hypotheses for incident with {len(incident.get('alerts', []))} alerts")
    ...
    def _apply_rules(self, incident: dict) -> list[RankedHypothesis]:
        ...
        if recent_deploy and any(kw in alerts_text for kw in ["error", "5xx", "failure"]):
        ...
        if latency > 0 and cpu < 50 and latency > 500:
            hypotheses.append(RankedHypothesis(
                ...
                evidence=[f"P99 latency {latency}ms with CPU only {cpu}% — I/O bound"],
    ...
    def _build_prompt(
        self, incident: dict, service_graph: dict | None, rule_hypotheses: list[RankedHypothesis]
    ) -> str:
        import json
        parts = []
    ...
    def _parse_hypotheses(self, text: str, rule_hypotheses: list[RankedHypothesis]) -> list[RankedHypothesis]:
        ...
        for hypothesis_text, confidence_str in pairs[:5]:
            try:
                ...
                if not any(h.hypothesis.lower()[:50] == hypothesis_text.strip().lower()[:50] for h in hypotheses):
    ...
    def _classify_category(self, text: str) -> str:
        ...
        t = text.lower()
        if any(w in t for w in ["database", "postgres", "mysql", "query", "connection"]):
    ...
    def _build_hypothesis_set(self, hypotheses: list[RankedHypothesis]) -> HypothesisSet:
        if not hypotheses:
